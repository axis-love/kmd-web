...
This document is a visual fixture for the Markdown reader. It exercises
the full feature set supported by `@axis-love/kmd-web`: paragraphs and
inline styles, lists, task lists, blockquotes, GitHub alerts, code with
syntax highlighting, tables, Mermaid diagrams, math (inline and display),
footnotes, and raw-HTML details.
...
flowchart TD
  ...
  Source --> Parse[Parse to AST]
  Parse --> Detect{Features?}
  Detect -->|has code| Highlight["Shiki highlight"]
  Detect -->|has math| Math["KaTeX render"]
  Detect -->|has diagram| Mermaid["Mermaid → SVG"]
  Highlight --> Safe["Sanitize → safe HTML"]
  Math --> Safe
  Mermaid --> Safe
  Safe --> Morph["DOM morph into reader"]
  Morph --> Done([Reader updated])
```
