...
- **Stroke tiers**: a diagram line is a 1px hairline on a large canvas, so strokes get two strengths. Everything that carries meaning as a line — flowchart edges and arrowheads, node outlines, sequence lifelines and signals — uses `--kmd-color-on-surface`, the body text color (~14:1 on dark, ~17:1 on light against the page). Containers whose fill already separates them — clusters, notes, label boxes — use `--kmd-color-secondary` (~6.5:1 dark, ~5.2:1 light). `--kmd-color-border` is never used for diagram geometry: it is tuned for large quiet surfaces and lands near 1.6:1 against the dark background. Node fills use `--kmd-color-surface-muted`, never `--kmd-color-primary`, which is a foreground color in kmd. The thresholds behind these choices are asserted per variable, in both themes, in `packages/mermaid/src/theme.test.ts`.
