...
class LookupIndex {
  ...
  private findInGroup(g: Map<string, string>, name: string): string | undefined {
    // 1. Exact match
...
export function resolveSpec(doc: DesignDocument): void {
  ...
  if (doc.spec.componentRecipes) {
    for (const recipe of doc.spec.componentRecipes) {
      ...
      for (const [key, value] of Object.entries(recipe.properties)) {
        ...
        if (typoMatch) {
          ...
          if (typoObj) {
            for (const [tKey, tVal] of Object.entries(typoObj)) {
              ...
              additions[camelKey] = resolveValueInner(tVal, index, new Set(), diagnostics);
            }
            removals.push(key);
            continue;
          }
        ...
        recipe.properties[key] = resolveValueInner(value, index, new Set(), diagnostics);
      }
  ...
  // -----------------------------------------------------------------------
  const tokenArrays: Array<Array<{ name: string; value: string }> | undefined> = [
  ...
  for (const tokens of tokenArrays) {
    ...
    for (const token of tokens) {
      token.value = resolveValueInner(token.value, index, new Set(), diagnostics);
    }
  }
