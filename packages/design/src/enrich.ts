...
import type { ColorGroup, ColorRole, ColorToken, DesignDocument } from "./ir.js";
...
function normaliseTokenName(name: string): string {
  return name.replace(/^(--)?color[-_]?/i, "").replace(/[-_]/g, " ");
}
...
function inferPairs(tokens: readonly ColorToken[]): void {
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    for (let j = i + 1; j < tokens.length; j++) {
      ...
      const shared = bareA.some((w) => w.length > 2 && bareB.includes(w));
      if (shared) {
        a.pair = b.name;
        b.pair = a.name;
        break;
 ...
 */
function findCandidate(byName: Map<string, ColorToken>, bare: string): ColorToken | undefined {
  // Normalise bare name back to common token formats.
  const kebab = bare.replace(/\s+/g, "-");
  const candidates = [kebab, `color-${kebab}`, `--color-${kebab}`];
...
function hasSubstituteFromRaw(doc: DesignDocument, tokenName: string): boolean {
