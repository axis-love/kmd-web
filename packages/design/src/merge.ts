...
function mergeTokenArray<T extends NamedToken>(
  ...
  for (const [, group] of byName) {
    ...
    const sorted = [...group].sort((a, b) => {
      const precDiff = precedenceOf(b.provenance.extractor) - precedenceOf(a.provenance.extractor);
 ...
 */
function formatConflict(tokenName: string, winner: NamedToken, loser: NamedToken): string {
...
function mergeComponentRecipes(recipes: readonly ComponentRecipe[]): {
  merged: ComponentRecipe[];
  ...
  for (const [, group] of byName) {
    ...
    const sorted = [...group].sort(
      (a, b) => precedenceOf(b.provenance.extractor) - precedenceOf(a.provenance.extractor),
    );
    ...
    for (let i = 1; i < sorted.length; i++) {
      ...
      for (const [key, loserVal] of Object.entries(loser.properties)) {
        ...
        if (winnerVal != null && normalizeValue(winnerVal) !== normalizeValue(loserVal)) {
          const wLine =
            winner.provenance.sourceLine != null ? ` (line ${winner.provenance.sourceLine})` : "";
          const lLine =
            loser.provenance.sourceLine != null ? ` (line ${loser.provenance.sourceLine})` : "";
          conflicts.push({
            severity: "warning",
...
function mergeGradientTokens(tokens: readonly GradientToken[]): {
...
function mergeIconSetHints(hints: readonly IconSetHint[]): {
  merged: IconSetHint[];
  conflicts: Diagnostic[];
} {
...
export function mergeSpecs(partials: readonly DesignSpec[]): {
  ...
  for (const partial of partials) {
    ...
    if (partial.typographyTokens) allTypographyTokens.push(...partial.typographyTokens);
    ...
    if (partial.elevationTokens) allElevationTokens.push(...partial.elevationTokens);
    ...
    if (partial.componentRecipes) allComponentRecipes.push(...partial.componentRecipes);
    if (partial.gradientTokens) allGradientTokens.push(...partial.gradientTokens);
    if (partial.motionTokens) allMotionTokens.push(...partial.motionTokens);
    if (partial.breakpointTokens) allBreakpointTokens.push(...partial.breakpointTokens);
