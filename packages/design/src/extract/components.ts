import type { Heading, List, ListItem, Paragraph, Parents, PhrasingContent, Root } from "mdast";
import { fromMarkdown } from "mdast-util-from-markdown";
import { visit } from "unist-util-visit";
import type { ComponentRecipe, DesignDocument, Provenance } from "../ir.js";
import { splitYamlFrontMatter } from "./yaml.js";
...
const KEY_ALIASES: ReadonlyArray<{ patterns: RegExp[]; normalized: string }> = [
  ...
  {
    patterns: [/^hoverbackgroundcolor$/i, /^hoverbg$/i, /^hovercolor$/i, /^hoverbackground$/i],
    normalized: "hover-background",
  },
...
function extractFocusProps(text: string): Record<string, string> {
  ...
  const focusRing =
    /\bfocus:\s*(?:border\s+)?(?:#[0-9a-fA-F]{3,8}\s*,?\s*)?ring\s+(\d+(?:\.\d+)?px)\s+(#[0-9a-fA-F]{3,8})/gi;
  ...
  const focusRingOffset =
    /\bfocus:\s*border\s+#[0-9a-fA-F]{3,8},?\s*ring\s+\d+(?:\.\d+)?px\s+(?:offset\s+)?(?:#[0-9a-fA-F]{3,8})?/gi;
...
const INLINE_PROP_PATTERNS: ReadonlyArray<{
  ...
  {
    pattern: /padding:\s*([\d]+(?:\.\d+)?(?:px\s*)+(?:[\d]+(?:\.\d+)?(?:px\s*)*)?)/i,
    key: "padding",
  },
...
function extractParensHex(text: string): Array<{ key: string; value: string }> {
...
function extractInlineProps(text: string): Record<string, string> {
  ...
  const namedColorRef = /\b(?:Text\s+\w+|Border|Surface)\s+\w+\s*\((#[0-9a-fA-F]{3,8})\)/g;
  ...
  const elevMatch = /\b(Small|Medium|Large|Extra\s*Large|Subtle|Overlay)\s+elevation\b/i.exec(text);
  if (elevMatch) {
...
const STATE_KEYWORDS = ["hover", "active", "focus", "disabled", "selected", "error"] as const;

function extractStates(text: string): {
  ...
  for (const state of STATE_KEYWORDS) {
    const stateRe = new RegExp(`\\b${state}:\\s*([^,.]+?)(?:[,.]|$)`, "gi");
...
function parsePropertyBulletList(
  ...
  return Object.keys(props).length > 0 ||
    Object.keys(hoverProps).length > 0 ||
    Object.keys(focusProps).length > 0
...
function buildRecipe(
  ...
  const hasStructured = Object.keys(properties).some((k) => k !== "family" && k !== "description");
  ...
  visit(tree, (node, _index, parent) => {
    ...
    if (node.type === "listItem") {
      ...
      if (item.children.length >= 2) {
        ...
        const firstList = item.children.find((c): c is List => c.type === "list");
        if (firstPara && firstList && !visitedParentLists.has(firstList)) {
          ...
          if (componentName) {
            ...
            if (bulletResult) {
              ...
              doc.spec.componentRecipes = [...(doc.spec.componentRecipes ?? []), recipe];
              return;
            }
          }
        }
