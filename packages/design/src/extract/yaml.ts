...
function buildColorTokens(raw: Record<string, unknown>, lineOffset: number): ColorToken[] {
...
function buildSpacingTokens(raw: Record<string, unknown>, lineOffset: number): SpacingToken[] {
...
function buildRadiusTokens(raw: Record<string, unknown>, lineOffset: number): RadiusToken[] {
...
function buildLayoutTokens(raw: Record<string, unknown>, lineOffset: number): LayoutToken[] {
  ...
  for (const [yamlKey, category] of Object.entries(CATEGORY_MAP)) {
    ...
    switch (category) {
      ...
      case "elevation": {
        ...
        doc.spec.elevationTokens = buildSpacingTokens(map, offset).map((t) => ({
          name: t.name,
          value: t.value,
      ...
      case "surface": {
        ...
        doc.spec.surfaceTokens = buildSpacingTokens(map, offset).map((t) => ({
          name: t.name,
          value: t.value,
          provenance: t.provenance,
        }));
