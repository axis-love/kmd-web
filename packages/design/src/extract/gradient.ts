...
import type { DesignDocument, GradientToken, Provenance } from "../ir.js";
...
const GRADIENT_RE = /(?:linear-gradient|radial-gradient)\s*\((?:[^()]+|\([^()]*\))+\)/gi;
 ...
 */
function scanCssBlocks(tree: Root, bodyLineOffset: number, existingCount: number): GradientToken[] {
...
export function extractGradient(doc: DesignDocument): void {
  ...
  visit(tree, (node) => {
    if (node.type === "listItem") {
      const para = (
        node as unknown as { children: Array<{ type: string; children?: PhrasingContent[] }> }
