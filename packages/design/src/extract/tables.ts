...
import type { Heading, PhrasingContent, Root, Table, TableCell, TableRow } from "mdast";
...
const SECTION_PATTERNS: ReadonlyArray<{
  ...
  {
    category: "component",
    pattern: /\bcomponents?\b|\bbuttons?\b|\bcards?\b|\binputs?\b|\bchips?\b|\bbadges?\b/,
  },
];
...
function buildColumnMapping(headerRow: TableRow): ColumnMapping | undefined {
  ...
  for (let i = 0; i < cells.length; i++) {
    ...
    else if (role === "letterSpacing" && mapping.letterSpacing === undefined)
      mapping.letterSpacing = i;
  }
...
function extractColorTokens(
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    if (description) {
      token.description = paletteGroup ? `${paletteGroup}: ${description}` : description;
    } else if (paletteGroup) {
      token.description = paletteGroup;
    }
    const colorGroup = paletteGroup ? paletteGroupToColorGroup(paletteGroup) : undefined;
    if (colorGroup) {
      token.group = colorGroup as import("../ir.js").ColorGroup;
    }
    tokens.push(token);
  }
  return tokens;
...
function extractComponentRecipes(
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    for (let c = 0; c < row.children.length; c++) {
      ...
      const headerText = phrasingText(headerRow.children[c]?.children ?? ([] as PhrasingContent[]));
  ...
  visit(tree, (node) => {
    ...
    switch (currentCategory) {
      ...
      case "typography": {
        ...
        doc.spec.typographyTokens = [...(doc.spec.typographyTokens ?? []), ...tokens];
        break;
      }
      case "spacing": {
      ...
      case "elevation": {
        ...
        doc.spec.elevationTokens = [...(doc.spec.elevationTokens ?? []), ...tokens];
        break;
      }
      case "layout": {
      ...
      case "component": {
        ...
        doc.spec.componentRecipes = [...(doc.spec.componentRecipes ?? []), ...recipes];
        break;
      }
    }
  });
