...
import type { DesignDocument, Provenance, SurfaceToken } from "../ir.js";
...
function buildColumnMapping(headerRow: TableRow): ColumnMapping | undefined {
  ...
  for (let i = 0; i < cells.length; i++) {
    ...
    else if (role === "level" && mapping.level === undefined) mapping.level = i;
...
function extractTokensFromTable(table: Table, lineOffset: number): SurfaceToken[] {
