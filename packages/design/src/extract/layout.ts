...
import type { Heading, ListItem, Paragraph, PhrasingContent, Root } from "mdast";
import { fromMarkdown } from "mdast-util-from-markdown";
import { visit } from "unist-util-visit";
import type { DesignDocument, LayoutToken, Provenance } from "../ir.js";
...
function extractInlineBoldTokensFromPara(para: Paragraph, line: number): LayoutToken[] {
  ...
  for (let i = 0; i < children.length; i++) {
    ...
    for (let j = i + 1; j < children.length; j++) {
      ...
      if (next.type === "text") {
        ...
        restParts.push(phrasingText((next as { children: PhrasingContent[] }).children));
      }
    }

    const value = restParts
      .join("")
      .replace(/^\s*\|\s*/, "")
      .replace(/\s*\|\s*$/, "")
      .trim();
...
export function extractLayout(doc: DesignDocument): void {
  ...
  visit(tree, (node, _index, parent) => {
    ...
    if (node.type === "paragraph" && parent?.type !== "listItem") {
      ...
      const strongCount = para.children.filter((c) => c.type === "strong").length;
