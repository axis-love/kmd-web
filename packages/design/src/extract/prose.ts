...
import type { Heading, ListItem, Paragraph, Parents, PhrasingContent, Root } from "mdast";
 ...
 *  "(#HEX)"               →  value=HEX
 */
function extractColorToken(name: string, rest: string, line: number): ColorToken | undefined {
...
export function extractProse(doc: DesignDocument): void {
  ...
  visit(tree, (node) => {
      ...
      switch (currentCategory) {
        case "color": {
          ...
          if (token) {
            ...
            doc.spec.colorTokens = [...(doc.spec.colorTokens ?? []), token];
        ...
        case "typography": {
          ...
          if (token) {
            doc.spec.typographyTokens = [...(doc.spec.typographyTokens ?? []), token];
          }
          break;
        }
        case "spacing": {
          ...
          doc.spec.spacingTokens = [...(doc.spec.spacingTokens ?? []), token];
        ...
        case "radius": {
          ...
          doc.spec.radiusTokens = [...(doc.spec.radiusTokens ?? []), token];
        ...
        case "elevation": {
          ...
          doc.spec.elevationTokens = [...(doc.spec.elevationTokens ?? []), token];
          break;
        }
        case "layout": {
    ...
    if (node.type === "paragraph" && currentCategory === "color") {
      ...
      if (para.children.length > 0 && para.children[0]!.type === "strong") return;
