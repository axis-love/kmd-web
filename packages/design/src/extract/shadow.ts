...
import type { Code, Heading, ListItem, Paragraph, PhrasingContent, Root } from "mdast";
import { fromMarkdown } from "mdast-util-from-markdown";
import { visit } from "unist-util-visit";
import type { DesignDocument, Provenance } from "../ir.js";
...
export function extractShadow(doc: DesignDocument): void {
  ...
  visit(tree, (node) => {
    ...
    if (node.type === "listItem") {
      ...
      const line = nodeLine(item.position?.start.line, 1, offset);
