...
interface RoutedColor extends ColorToken {
  _kind: "color";
}
interface RoutedTypography extends TypographyToken {
  _kind: "typography";
}
interface RoutedSpacing extends SpacingToken {
  _kind: "spacing";
}
interface RoutedRadius extends RadiusToken {
  _kind: "radius";
}
interface RoutedElevation extends ElevationToken {
  _kind: "elevation";
}
interface RoutedUnknown {
  _kind: "unknown";
  name: string;
  value: string;
...
function routeDeclaration(prop: string, value: string, sourceLine: number): RoutedToken {
  ...
  if (bare.startsWith("shadow-") || bare.startsWith("elevation-")) {
    return {
      _kind: "elevation",
      name: bare.startsWith("shadow-") ? bare.slice(7) : bare.slice(10),
      value,
      provenance,
    };
  }
