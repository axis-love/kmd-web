...
const DESIGN_SECTION_HEADING =
  /^\s{0,3}#{1,3}\s+(colors?|typography|spacing|radii?|radius|rounded?|elevation|shadows?|surfaces?|layout|components?|palette|tokens?)\b/gim;
const DESIGN_TOKEN_LINE =
  /^\s*(?:[-*+]\s*)?(?:--)?[a-z0-9][\w-]*\s*:\s*(?:#[0-9a-f]{3,8}|\{[^}]+\}|var\(--[\w-]+\)|[0-9.]+(?:px|rem|em|%)?|["'][^"']+["'])\s*$/gim;
...
export function hasDesignTokens(content: string): boolean {
  ...
  if (frontmatter.trim()) {
    const hasDesignKey =
      /^(?:colors?|typography|spacing|radii?|rounded?|elevation|surfaces?|layout|components?|palette)\s*:/m.test(
        frontmatter,
      );
...
export function summarizeMarkdownForDesignMode(content: string): DesignModeDocumentSummary {
  ...
  try {
    ...
    return {
      ...
      excerpt,
      headings,
      stats: { wordCount: 0, headingCount: 0, listCount: 0, codeBlockCount: 0 },
    };
  }
  ...
  visit(tree, (node) => {
    if (node.type === "heading") {
      ...
      const text = phrasingText(
        h.children as Array<{ type: string; value?: string; children?: unknown }>,
      ).trim();
      if (text) {
        headings.push(text);
        headingCount++;
    ...
    if (node.type === "paragraph") {
      ...
      const text = phrasingText(
        p.children as Array<{ type: string; value?: string; children?: unknown }>,
      ).trim();
      if (text) {
      ...
      for (const item of list.children) {
        for (const child of item.children) {
          if (child.type === "paragraph") {
            wordCount += phrasingText(
              (child as Paragraph).children as Array<{
                type: string;
                value?: string;
                children?: unknown;
              }>,
  ...
  return {
    ...
    excerpt,
    headings,
    stats: { wordCount, headingCount, listCount, codeBlockCount },
  };
}
...
export function parseProseDesignSpec(content: string): ProseDesignSpec {
  ...
  visit(tree, (node) => {
    ...
    if (node.type === "listItem") {
      ...
      if (!para) return;
      const text = phrasingText(
        para.children as Array<{ type: string; value?: string; children?: unknown }>,
      ).trim();
      if (!text) return;
      const target = currentSubsection ?? currentSection;
      if (target) {
