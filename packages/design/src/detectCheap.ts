...
const DESIGN_SYSTEM_PROSE_RE =
  /\b(?:design\s+system|design\s+tokens?|component\s+library|token\s+inventory)\b/i;
const COMPONENT_STYLE_RE =
  /`[^`]*(?:bg|background|text|border|radius|padding|height|font|hover|focus)\s*:\s*[^`]*`/gi;

export function detectDesignDocumentCheap(content: string, filename?: string): DetectionOutput {
