import type { ColorToken, ComponentRecipe, DesignDocument } from "./ir.js";
...
function isThemeTextOnVariant(combined: string, isDarkTheme: boolean): boolean {
  ...
  if (
    new RegExp(
      `\\b(?:text|body|ink|foreground|heading|head|muted|dim|caption|content)\\s+on\\s+${theme}\\b`,
    ).test(combined)
  ) {
    return true;
  }
...
function shouldRejectOnVariantForVar(
  token: ColorToken,
  nyxVar: NyxVar,
  isDarkTheme: boolean,
): boolean {
  ...
  if (
    isTextHeadRole(nyxVar) ||
    isTextBodyRole(nyxVar) ||
    isTextMutedRole(nyxVar) ||
    isTextDimRole(nyxVar)
  ) {
...
const ACCENT_HOVER_KEYWORDS: Array<{ pattern: RegExp; confidence: number }> = [
  {
    pattern: /\b(?:primary[\s-]?focus|primary[\s-]?hover|accent[\s-]?hover|accent[\s-]?focus)\b/,
    confidence: 100,
  },
  {
    pattern: /\b(?:action[\s-]?hover|action[\s-]?focus|hovered[\s-]?accent|hovered[\s-]?primary)\b/,
    confidence: 90,
  },
  {
    pattern:
      /\b(?:component[\s-]?hover|component[\s-]?focus|interactive[\s-]?hover|interactive[\s-]?focus)\b/,
    confidence: 85,
  },
  ...
  {
    pattern: /\b(?:cta[\s-]?hover|link[\s-]?hover|button[\s-]?hover|btn[\s-]?hover)\b/,
    confidence: 70,
  },
];
...
const BG_KEYWORDS: Array<{ pattern: RegExp; confidence: number }> = [
  ...
  {
    pattern:
      /\b(?:page[\s-]?background|app[\s-]?background|body[\s-]?background|global[\s-]?background)\b/,
    confidence: 95,
  },
...
function isPaletteAccentToken(token: ColorToken): boolean {
  ...
  if (role === "accent" && /\b(?:secondary|tertiary|quaternary)\b/.test(name) && !isTextLike)
    return true;
  if (token.group === "Brand" && /\b(?:secondary|tertiary)\b/.test(name) && !isTextLike)
    return true;
  return false;
}
...
function normalizeBorderValue(value: string): string | undefined {
  ...
  const borderMatch = trimmed.match(
    /^(\d+(?:\.\d+)?(?:px|em|rem)?)\s+(solid|dashed|dotted)?\s*(.+)$/i,
  );
  if (borderMatch) {
  ...
  if (isBgOrSurfaceVar(nyxVar)) {
    ...
    if (!isDarkTheme) {
      const combined =
        n +
        " " +
        (token.role ? normalizeTokenName(token.role) : "") +
        " " +
        (token.description ? normalizeTokenName(token.description) : "");
...
function invertThemeVars(
  ...
  for (const [key, value] of vars) {
    ...
    } else if (key.startsWith("--nyx-radius-") || key === "--nyx-font-family") {
      inverted.set(key, value);
    } else if (
      key.endsWith("-size") ||
      key.endsWith("-weight") ||
...
function mapRadiusToken(token: { name: string; value: string }): [string, string] | null {
  ...
  if (n.includes("button") || n.includes("btn")) return ["--nyx-radius-btn", v];
  ...
  if (
    isZeroRadius(v) &&
    (n === "none" || n === "default" || n === "base" || n === "all" || n === "global")
  ) {
...
function inferRecipeFamily(
  name: string,
  properties: Record<string, string>,
): ComponentFamily | undefined {
...
function isPrimaryButtonRecipe(name: string, properties: Record<string, string>): boolean {
  const combined = normalizeTokenName(
    `${name} ${properties.family ?? ""} ${properties.role ?? ""}`,
  );
  if (
    /\b(?:secondary|ghost|outline|utility|icon|link|pearl|circle|circular|nav)\b/.test(combined)
  ) {
    return false;
  }
...
function mapRecipeStyles(recipe: ComponentRecipe, doc: DesignDocument): [string, string][] {
  ...
  const background = pickRecipeProp(props, [
    "backgroundColor",
    "background-color",
    "background",
    "bg",
  ]);
  const foreground = pickRecipeProp(props, [
    "textColor",
    "text-color",
    "foreground",
    "color",
  ...
  const padding = pickRecipeProp(props, [
    "padding",
    "paddingX",
    "paddingY",
    "padding-x",
  ...
  const hoverBg = pickRecipeProp(props, [
    "hoverBackgroundColor",
    "hover-background",
    "hover-color",
    "state-hover",
  ]);
  const focusRing = pickRecipeProp(props, [
    "focusRing",
    "focus-ring",
    "focus-ring-color",
    "focusBorderColor",
  ...
  if (family === "input") {
    ...
    const placeholderColor = pickRecipeProp(props, [
      "placeholderColor",
      "placeholder-color",
      "placeholder",
    ]);
    const inputTextColor = pickRecipeProp(props, [
      "textColor",
      "text-color",
      "foreground",
      "color",
...
function parseTypoValue(value: string): ParsedTypography {
  ...
  const weightMatch = value.match(
    /\b(100|200|300|400|500|600|700|800|900|normal|bold|bolder|lighter|medium|semibold)\b/i,
  );
  ...
  const letterSpacingMatch = value.match(
    /(?:letter-spacing|letter spacing)\D*(-?[\d.]+(?:px|em|rem)?)/i,
  );
...
function deriveTypographyTheme(
  doc: DesignDocument,
): Partial<Record<TypographyRole, ParsedTypography>> {
...
function similarFontFallbackStack(
  ...
  if (
    primary &&
    /\b(?:poppins|montserrat|outfit|sora|space grotesk|dm sans|general sans|sohne|inter)\b/i.test(
      primary,
    )
  ) {
...
function buildTypographyVars(doc: DesignDocument): Map<string, string> {
  ...
  const headingFamily =
    theme.heading?.fontFamily ?? theme.display?.fontFamily ?? theme.body?.fontFamily ?? bodyFamily;
  const headingStack = withFontFallback(
    headingFamily,
    inferFontGeneric(headingFamily, "sans-serif"),
  );
  if (headingStack) {
...
const DARK_KEYWORDS: Array<{ pattern: RegExp; confidence: number }> = [
  ...
  {
    pattern: /\bdark[\s-]?(?:text|ink|foreground|heading|body|muted|dim|border|sep)\b/,
    confidence: 90,
  },
  {
    pattern: /\b(?:text|ink|foreground|heading|body|muted|dim|border|sep)[\s-]?dark\b/,
    confidence: 90,
  },
...
const LIGHT_KEYWORDS: Array<{ pattern: RegExp; confidence: number }> = [
  ...
  {
    pattern: /\blight[\s-]?(?:text|ink|foreground|heading|body|muted|dim|border|sep)\b/,
    confidence: 90,
  },
  {
    pattern: /\b(?:text|ink|foreground|heading|body|muted|dim|border|sep)[\s-]?light\b/,
    confidence: 90,
  },
...
function deriveOppositeVar(
  ...
  if (
    nyxVar.endsWith("-size") ||
    nyxVar.endsWith("-weight") ||
    nyxVar.endsWith("-line") ||
    nyxVar.endsWith("-track")
  )
    return undefined;
  ...
  if (darkMode) {
    if (nyxVar === "--nyx-bg" || nyxVar === "--nyx-surface")
      return darkenColorConservative(lightValue, nyxVar === "--nyx-bg" ? "bg" : "surface");
    if (nyxVar === "--nyx-surface-elevated") return "rgba(255,255,255,0.05)";
    if (nyxVar === "--nyx-text-head" || nyxVar === "--nyx-text-body")
      return darkenColorConservative(lightValue, "text");
    ...
    if (nyxVar === "--nyx-bg" || nyxVar === "--nyx-surface")
      return lightenColorConservative(lightValue, nyxVar === "--nyx-bg" ? "bg" : "surface");
    if (nyxVar === "--nyx-surface-elevated") return "rgba(0,0,0,0.04)";
    if (nyxVar === "--nyx-text-head" || nyxVar === "--nyx-text-body")
      return lightenColorConservative(lightValue, "text");
...
function classifyRecipeAffinity(recipe: ComponentRecipe, doc: DesignDocument): DarkLightAffinity {
  ...
  const background = resolveRecipeProp(recipe, doc, [
    "backgroundColor",
    "background-color",
    "background",
    "bg",
  ]);
  const foreground = resolveRecipeProp(recipe, doc, [
    "textColor",
    "text-color",
    "foreground",
    "color",
...
function addRecipeVarsForAffinity(target: Map<string, string>, entries: [string, string][]): void {
...
function pickThemeText(
  ...
  const keywords =
    nyxVar === "--nyx-text-muted"
      ? TEXT_MUTED_KEYWORDS
      : nyxVar === "--nyx-text-dim"
        ? TEXT_DIM_KEYWORDS
        : nyxVar === "--nyx-text-body"
          ? TEXT_BODY_KEYWORDS
          : TEXT_HEAD_KEYWORDS;
  const token = pickBestForVar(
    colorTokens.filter((candidate) => {
...
function repairThemePolarity(
  ...
  if (isDarkTheme) {
    if (isOpaqueLightColor(vars.get("--nyx-bg")) === true) {
      vars.set(
        "--nyx-bg",
        pickThemeSurfaceOrBackground(colorTokens, "--nyx-bg", true) ?? "#000000",
      );
    }
    if (isOpaqueLightColor(vars.get("--nyx-surface")) === true) {
      vars.set(
        "--nyx-surface",
        pickThemeSurfaceOrBackground(colorTokens, "--nyx-surface", true) ??
    ...
    if (isOpaqueDarkColor(vars.get("--nyx-text-muted")) === true) {
      vars.set(
        "--nyx-text-muted",
        pickThemeText(colorTokens, "--nyx-text-muted", true) ?? "rgba(255,255,255,0.6)",
      );
    }
    if (isOpaqueDarkColor(vars.get("--nyx-text-dim")) === true) {
      vars.set(
        "--nyx-text-dim",
        pickThemeText(colorTokens, "--nyx-text-dim", true) ?? "rgba(255,255,255,0.35)",
      );
    }
  ...
  if (isOpaqueDarkColor(vars.get("--nyx-surface")) === true) {
    vars.set(
      "--nyx-surface",
      pickThemeSurfaceOrBackground(colorTokens, "--nyx-surface", false) ?? "#ffffff",
    );
  }
  for (const key of ["--nyx-text-head", "--nyx-text-body"] as const) {
  ...
  if (isOpaqueLightColor(vars.get("--nyx-text-muted")) === true) {
    vars.set(
      "--nyx-text-muted",
      pickThemeText(colorTokens, "--nyx-text-muted", false) ?? "rgba(0,0,0,0.6)",
    );
  }
  if (isOpaqueLightColor(vars.get("--nyx-text-dim")) === true) {
    vars.set(
      "--nyx-text-dim",
      pickThemeText(colorTokens, "--nyx-text-dim", false) ?? "rgba(0,0,0,0.35)",
    );
  }
...
/** Light and dark `--nyx-*` variable maps extracted from a design document. */
...
/**
 * Build the showcase theme variables for a design document: the single
 * token extractor shared by kmd's design-mode showcase and the reader
 * theme emitter (emitThemeTokens). Returns null when the document yields
 * nothing themeable.
 */
export function buildShowcaseThemeVars(doc: DesignDocument): ShowcaseThemeVars | null {
  ...
  for (const mapping of COLOR_MAPPINGS) {
    ...
    const lightSpecificBest = pickBestForVar(
      lightSpecificTokens,
      mapping.nyxVar,
      mapping.keywords,
      false,
    ...
    const darkSpecificBest = pickBestForVar(
      darkSpecificTokens,
      mapping.nyxVar,
      mapping.keywords,
      true,
  ...
  // Reader radius scale (KWEB-068). The component radii above drive the
  // showcase's buttons, cards and chips; a document reader themes by size,
  // so size-named radius tokens (sm/md/lg/xl/full and their long forms) map
  // 1:1 onto --nyx-radius-<size>, and a global radius fills the gaps.
  const RADIUS_SCALE: ReadonlyArray<readonly [RegExp, string]> = [
    [/(?:^|[-_\s/.])(?:sm|small)$/, "sm"],
    [/(?:^|[-_\s/.])(?:md|medium)$/, "md"],
    [/(?:^|[-_\s/.])(?:lg|large)$/, "lg"],
    [/(?:^|[-_\s/.])(?:xl|x[-_\s]?large|extra[-_\s]?large)$/, "xl"],
    [/(?:^|[-_\s/.])(?:full|pill|circle)$/, "full"],
  ];
  const RADIUS_LENGTH = /^\d+(?:\.\d+)?(?:px|rem|em|%)$/;
  const scaleRadiusVars = new Map<string, string>();
  for (const token of radiusTokens) {
    const n = token.name.toLowerCase().trim();
    const v = normalizeRadiusValue(token.value);
    if (!RADIUS_LENGTH.test(v)) continue;
    for (const [pattern, size] of RADIUS_SCALE) {
      const key = `--nyx-radius-${size}`;
      if (pattern.test(n) && !scaleRadiusVars.has(key)) scaleRadiusVars.set(key, v);
    }
  }
  // A global zero ("all: 0px", or a bare "none") fills the gaps only when the
  // design declares no non-zero size radius — Apple's "none: 0px" next to
  // "sm: 8px" is a scale step, not a sharp-corners policy.
  const hasNonZeroScale = [...scaleRadiusVars].some(
    ([k, v]) => k !== "--nyx-radius-full" && !isZeroRadius(v),
  );
  if (hasGlobalZeroRadius && !hasNonZeroScale) {
    for (const size of ["sm", "md", "lg", "xl"]) {
      const key = `--nyx-radius-${size}`;
      if (!scaleRadiusVars.has(key)) scaleRadiusVars.set(key, "0px");
    }
  } else if (globalRadiusValue) {
    for (const size of ["md", "lg"]) {
      const key = `--nyx-radius-${size}`;
      if (!scaleRadiusVars.has(key)) scaleRadiusVars.set(key, globalRadiusValue);
    }
  }
  for (const [k, v] of scaleRadiusVars) sharedRecipeVars.set(k, v);
  ...
  if (!lightVars.has("--nyx-bg")) {
    lightVars.set(
      "--nyx-bg",
      pickThemeSurfaceOrBackground(colorTokens, "--nyx-bg", false) ?? "#f5f5f7",
    );
  }
  if (!lightVars.has("--nyx-surface")) {
    lightVars.set(
  ...
  if (!lightVars.has("--nyx-text-head")) {
    lightVars.set(
      "--nyx-text-head",
      pickThemeText(colorTokens, "--nyx-text-head", false) ?? "#1d1d1f",
    );
  }
  if (!lightVars.has("--nyx-text-body")) {
    lightVars.set(
      "--nyx-text-body",
      pickThemeText(colorTokens, "--nyx-text-body", false) ??
        lightVars.get("--nyx-text-head") ??
        "rgba(0,0,0,0.72)",
    );
  }
  ...
  if (!darkVars.has("--nyx-surface")) {
    darkVars.set(
      "--nyx-surface",
      pickThemeSurfaceOrBackground(colorTokens, "--nyx-surface", true) ?? "rgba(255,255,255,0.05)",
    );
  }
  if (!darkVars.has("--nyx-text-head")) {
    darkVars.set(
      "--nyx-text-head",
      pickThemeText(colorTokens, "--nyx-text-head", true) ?? "#ffffff",
    );
  }
  if (!darkVars.has("--nyx-text-body")) {
    darkVars.set(
      "--nyx-text-body",
      pickThemeText(colorTokens, "--nyx-text-body", true) ??
        darkVars.get("--nyx-text-head") ??
  ...
  if (!hasExplicitDark) {
    ...
    if (!darkVars.has("--nyx-surface") && lightVars.has("--nyx-surface")) {
      derivedDark.set(
        "--nyx-surface",
        darkenColorConservative(lightVars.get("--nyx-surface")!, "surface"),
      );
    }
...
function darkColorVarsHasContent(
  ...
  if (
    darkVars.has("--nyx-accent") &&
    lightVars.get("--nyx-accent") !== darkVars.get("--nyx-accent")
  )
    return true;
  return false;
}
...
function addPrimaryFontFamilyName(
  names: Map<string, string>,
  familyStack: string | undefined,
): void {
