...
import { morphMarkdownBody, RAW_IMAGE_SRC_ATTR } from "./dom-morph";
...
describe("morphMarkdownBody", () => {
  ...
  it("handles empty nextHtml by clearing container", () => {
    const container = bodyWith("<p>One</p><p>Two</p>");
    const changed = morphMarkdownBody(container, "");
    expect(changed).toBe(true);
    expect(container.childNodes).toHaveLength(0);
  });

  it("handles empty container with content to add", () => {
    const container = bodyWith("");
    const changed = morphMarkdownBody(container, "<p>Hello</p>");
    expect(changed).toBe(true);
